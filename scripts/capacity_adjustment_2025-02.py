...
if iteration > int(snakemake.config["EVA"]["initial_phase"]):           
    adjustment_factor = snakemake.config["EVA"]["adjustment_factor_saturation"]
...
capacity_adjustment_new = pd.Series(index=revenue_ratios_new.index)
capacity_adjustment_new.loc[revenue_ratios_new>1] = capacity_change
capacity_adjustment_new.loc[revenue_ratios_new<1] = - capacity_change    

revenue_ratios_existing = revenue_ratios_existing_plants(average_expected_revenue)

capacity_adjustment_existing = pd.DataFrame(0, revenue_ratios_existing.index, revenue_ratios_existing.columns)
capacity_adjustment_existing[revenue_ratios_existing >1] = capacity_change
capacity_adjustment_existing[revenue_ratios_existing <1] = -capacity_change
